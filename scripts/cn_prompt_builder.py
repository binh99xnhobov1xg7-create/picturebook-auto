...
from config import (
    ...
    STYLE_PROFILE as _STYLE_PROFILE,
...
# 画风档（用户拍板 2026-06-12）：clean=清透光滑低纹理高清晰（去墨线/纸纹/水痕）；其余=水彩。
# 中文正文里的画风锚点（§7/§17/§18/§19 与人物画风锚）按此切换，杜绝残留 watercolor/细墨线/纸纹。
_STYLE_IS_CLEAN = (_STYLE_PROFILE == "clean")
# 行内画风用词（随档切换，避免 clean 档残留"水彩"字样）
_STYLE_WORD_CN = "干净平滑的现代绘本插画" if _STYLE_IS_CLEAN else "干净平滑统一的治愈水彩"
...
if _STYLE_IS_CLEAN:
    TARGET_CHARACTER_RENDER_CN = (
        "【目标人物画风锚·参考样张】人物按明亮干净的现代儿童绘本插画渲染（光滑平涂+柔和渐变、低纹理高清晰、"
        "无墨线痕/无纸纹/无水痕）：圆润稳定的可爱童书脸、圆润柔和的下巴、大而干净有神的暖色眼睛、小鼻子、"
        "干净柔和脸颊、自然儿童比例；姿势落地真实（坐/跪/俯身/站在地面或池畔），不漂浮、不四肢夸张；"
        "背景像样张一样舒适真实、明亮整洁、不拥挤。Mia/Tommy 跨页保持同一张脸。"
    )
else:
    TARGET_CHARACTER_RENDER_CN = (
        "【目标人物画风锚·参考样张】人物按温暖明亮经典儿童绘本水彩渲染：圆润稳定的可爱童书脸、"
        "大而有神的暖色眼睛、小鼻子、柔和脸颊、自然儿童比例；姿势落地真实（坐/跪/俯身/站在地面或池畔），"
        "不漂浮、不四肢夸张；背景像样张一样舒适真实、明亮整洁、不拥挤。Mia/Tommy 跨页保持同一张脸。"
    )
...
def _extra_color_assignment_cn() -> str:
    ...
    seq = "/".join(_EXTRA_COLOR_WHEEL)
    reserved = "、".join(_LEAD_RESERVED_COLORS)
    return (
        f"【配角配色·硬指派】配角按出场顺序循环取色（{seq}，相邻必不同、主色互不相同）；"
        f"严禁穿主角专属色：{reserved}（紫=Mia、蓝=Tommy、绿=Anna、粉=Cate），不与主角撞色；"
        + ("上色仍光滑柔和、过渡自然，不画成生硬色块。" if _STYLE_IS_CLEAN else "上色仍像水彩般柔和，不画成生硬色块。")
    )
...
def _nf_intro_page(page: PageSpec, outline: BookOutline) -> bool:
    """非虚构：【仅封面】保留 Mia+Tommy 作连续系列 IP 品牌（用户拍板 2026-06-11）。

    旧版把 P1 也算引子页（带双主角），但用户明确：科普书里 Mia/Tommy 只在【封面】出现，
    向小读者标示"这是同一系列"；所有正文页（含 P1）一律呈现【该主题/年代真实的人物与场景】，
    绝不把 Mia/Tommy 当成正文里的故事角色。
    """
    return _is_nonfiction(outline) and (page.page_type == "cover" or page.index == 0)


def _nf_body_page(page: PageSpec, outline: BookOutline) -> bool:
    """非虚构正文内页（除封面外的所有页 index≥1）：无主角，呈现本主题/年代真实人物与场景。

    用户拍板 2026-06-11：P1 也属正文（不再保留双主角引子），与 P2-P7 同样剔除 Mia/Tommy。
    """
...
def _lead_keys_for_book(outline: BookOutline, page: PageSpec | None = None) -> list[str]:
    """这本书在【有人物的页/封面】上应出现的系列主角键（四规则自动判定，用户拍板 2026-06-08）。
      ...
      规则4 科普【仅封面】→ Mia & Tommy 双主角作系列 IP 品牌；科普所有正文页(P1-P7) → 无主角（用户拍板 2026-06-11）。
...
# ============================================================
#  通用文本理解层 ①：关键贯穿物件统一（SOP「关键贯穿物件统一描述」）
#  通用对所有书生效，由 AI 抽取层(ai_extractor.key_object)判定"全书是否围绕同一关键物件"；
#  这里只把该唯一描述【逐页原样复用】，并明确"抽象描述(味道/气味…)不改变外观、只允许具体状态变化"。
# ============================================================
def _key_object_note(outline: BookOutline, page: PageSpec) -> str:
    """本页若涉及【全书贯穿的同一关键物件】，返回一段"同一物体·逐页一致·只改状态"的硬指令。

    完全由 outline.key_object（AI 抽取，present=True 才有）驱动，未命中返回空串（不影响其它书）。
    """
    ko = getattr(outline, "key_object", None)
    if not isinstance(ko, dict) or not ko.get("present"):
        return ""
    desc = str(ko.get("unified_desc_cn") or "").strip()
    if not desc:
        return ""
    idx = getattr(page, "index", 0)
    pages = ko.get("pages") or []
    # 给了出现页清单就只在这些页注入；没给则默认每一页都复用（保守地把"同一物件"贯穿全书）。
    if pages and idx not in pages:
        return ""
    name = str(ko.get("name") or "关键物件").strip()
    state = ""
    for s in (ko.get("states") or []):
        if isinstance(s, dict) and idx in (s.get("pages") or []):
            state = str(s.get("state") or "").strip()
            break
    note = (
        f"【关键贯穿物件·全书同一物·EXACT SAME·最高优先】本书贯穿同一关键物件＝{name}：{desc.rstrip('。')}。"
        "全书每页都是同一个物体，盛器/盖子/造型/颜色/大小/材质/边纹/摆放关系必须一致；"
        "配套盖子也固定，绝不换材质/颜色/形状/位置逻辑；"
        "文字里的味道/气味/感觉等抽象描述只是感受，绝不改变其外观、绝不在不同页画成不同食物/物体。"
    )
    extra = []
    if ko.get("orientation"):
        extra.append(f"固定朝向/摆放＝{str(ko.get('orientation')).strip()}")
    if ko.get("binding"):
        extra.append(f"装订/开合＝{str(ko.get('binding')).strip()}")
    if ko.get("content_visibility"):
        extra.append(f"内容可见性＝{str(ko.get('content_visibility')).strip()}")
    if extra:
        note += "；".join(extra) + "；这些跨页不得漂移。"
    if state:
        note += f"本页状态＝{state}；仅允许这一种物理状态变化，其余外观不变。"
    return note + "。"


def _recurring_props_note(outline: BookOutline, page: PageSpec) -> str:
    """多页普通道具统一锁：朝向/装订/内容朝内不可读等，避免笔记本/地图/课本逐页漂移。"""
    idx = getattr(page, "index", 0)
    notes: list[str] = []
    for prop in (getattr(outline, "recurring_props", None) or []):
        if not isinstance(prop, dict):
            continue
        pages = prop.get("pages") or []
        if pages and idx not in pages:
            continue
        name = str(prop.get("name") or "贯穿道具").strip()
        desc = str(prop.get("unified_desc_cn") or "").strip()
        if not desc:
            continue
        bits = [
            f"【贯穿道具·全书同一物·EXACT SAME】{name}：{desc.rstrip('。')}；每次出现都是同一个道具，"
            "颜色/大小/材质/形状/摆放逻辑一致"
        ]
        if prop.get("orientation"):
            bits.append(f"固定朝向/摆放＝{str(prop.get('orientation')).strip()}")
        if prop.get("binding"):
            bits.append(f"装订/折痕/开合＝{str(prop.get('binding')).strip()}")
        if prop.get("content_visibility"):
            bits.append(f"内容可见性＝{str(prop.get('content_visibility')).strip()}")
        for s in (prop.get("states") or []):
            if isinstance(s, dict) and idx in (s.get("pages") or []):
                st = str(s.get("state") or "").strip()
                if st:
                    bits.append(f"本页状态＝{st}")
                break
        notes.append("；".join(bits) + "。")
    pstate = (getattr(page, "prop_state", "") or "").strip()
    if pstate:
        notes.append(f"【本页道具状态补充】{pstate.rstrip('。')}。道具内容若朝向观众，只能是抽象线条/色块，绝不出现可读文字/字母/数字。")
    return "".join(notes)


def _class_ensemble_note(outline: BookOutline, page: PageSpec) -> str:
    """固定班级群像：exactly 4 classmates + 1 teacher，防 Students 被画成单个克隆角色。"""
    ens = getattr(outline, "class_ensemble", None)
    if not isinstance(ens, dict) or not ens.get("present") or page.index == 0:
        return ""
    era_kind, _ = _page_era(outline, page)
    if _is_nonfiction(outline) and era_kind in ("past", "modern", "contrast"):
        return ""
    text = _page_full_text(outline, page)
    if not _re.search(r"class|classmate|student|students|teacher|school|课堂|同学|学生|老师|教室", text, _re.I):
        return ""
    teacher = str(ens.get("teacher_desc_cn") or "").strip()
    mates = [m for m in (ens.get("classmates") or []) if isinstance(m, dict) and str(m.get("desc_cn") or "").strip()]
    bits = [
        "【班级群像·本书内部固定·最高优先】本书 classroom/class ensemble 固定为 exactly 4 个彼此不同的 classmates + 1 位 teacher；只在本书内复用，不跨书复用；严禁把 Students/class 画成一个单一角色或一排克隆同脸孩子。"
    ]
    if teacher:
        bits.append(
            f"teacher 固定外观：{teacher.rstrip('。')}。每次出现都是同一位老师，"
            "脸型/五官/发型/发色/肤色/身高体型/服装款式与配色完全一致；绝不换性别、年龄、发型、衣服、围裙状态或脸。"
        )
    if mates:
        mate_desc = "；".join(f"{m.get('id') or f'classmate_{i+1}'}＝{str(m.get('desc_cn')).rstrip('。')}" for i, m in enumerate(mates[:4]))
        bits.append(f"4 个 classmates 固定外观：{mate_desc}。四人脸型/发型/肤色/衣色各异，绝不撞 Mia/Tommy，也绝不彼此雷同；同一 classmate 跨页出现时脸/发/衣色完全一致。")
    return "".join(bits)


def _era_population_note(outline: BookOutline, page: PageSpec, era_kind: str) -> str:
    """非虚构今昔对比：同一 recurring cast 与不同时代人群分开处理。"""
    if not (_is_nonfiction(outline) and era_kind in ("past", "modern", "contrast")):
        return ""
    title_norm = _re.sub(r"[^a-z0-9]+", "", (getattr(outline, "title", "") or "").lower())
    text = _page_full_text(outline, page)
    if "school" not in title_norm and not _re.search(r"school|classroom|student|teacher|学校|教室|学生|老师", text, _re.I):
        return ""
    common = (
        "【非虚构今昔人群/工具区分】过去与现代不是同一批学生；"
        "两代人群的年龄、服装、发型、族裔、姿态和工具必须不同，绝不复制同一张脸或同一套学生。"
    )
    past = "过去：不同年龄孩子穿朴素旧衣，使用木桌、石板、粉笔、旧纸本、煤炉/油灯，绝无平板/电脑/投影。"
    modern = "现代：另一批多元孩子穿现代衣服，坐独立课桌，使用平板/电脑、现代课本、彩色文具、白板/投影，绝不拿石板当主工具。"
    if era_kind == "past":
        return common + past
    if era_kind == "modern":
        return common + modern
    return common + "分屏/并置时左旧右新或清晰分区：" + past + modern


def _thought_bubble_note(page: PageSpec) -> str:
    tb = (getattr(page, "thought_bubble", "") or "").strip()
    if not tb:
        return ""
    return (
        "【猜测/想象气泡·防提前剧透】现实场景中的 worker/person 仍保持本页真实中性衣着或普通日常衣着，"
        "绝不因为孩子的猜测提前穿上职业制服；只在画面一角画一个小型 thought/imagination bubble（半透明圆形气泡、占比小、不抢主体），"
        f"气泡里才出现想象内容：{tb.rstrip('。')}。气泡和现实人物必须视觉分离，观众一眼能看出这是猜测/想象，不是现实揭晓。"
    )


def _science_fiction_lock_note(outline: BookOutline, page: PageSpec) -> str:
    """白名单科学味 fiction 的画面真实性锁；当前仅 Book33 The Language of Dolphins。"""
    title_norm = _re.sub(r"[^a-z0-9]+", "", (getattr(outline, "title", "") or "").lower())
    if title_norm != "thelanguageofdolphins" or page.index == 0:
        return ""
    return (
        "【科学味 fiction 白名单·Book33 海豚场景真实性】本页发生在 ocean center 的开放宽阔 poolside 区域："
        "画面是池畔甲板 + 开阔训练池水面 + 低矮安全栏/池边，不是水族馆 acrylic glass 玻璃窗前观看；"
        "所有听声音/互动/记录页面都保持同一个 open/wide poolside set 连续，绝不切到玻璃隔窗或室内鱼缸展柜。"
        "海豚按真实友善海豚 anatomy：流线型身体、一个背鳍、两个胸鳍、水平尾鳍，比例正确；严禁多余鳍/多背鳍/鱼尾竖鳍。"
        "人物脚下是干燥 poolside deck，尤其 P6 不把 Mia/Tommy 画在水里。"
    )


# ============================================================
#  通用文本理解层 ②：科普/历史书的【本页年代/时代】匹配
#  典型坑：Book06《Schools of the Past》——讲"过去的学校"的页被画成现代教室
#  （现代环境提示 + 现代地点布景蓝本 把过去页拽成现代）。
#  解决：从本页文字(或 AI 的 page.era)推断年代，注入【年代锁】并覆盖与之冲突的"现代"描述；
#       过去/对比页同时关闭"现代教室"环境提示与现代地点布景蓝本，避免互相打架。
# ============================================================
_ERA_PAST_RE = _re.compile(
    r"in the past|long ago|years? ago|old(?:en)? days?|used to|back then|"
    r"in history|historical|ancient|centuries ago|decades ago|in the old days|"
    r"过去|从前|以前|很久以前|古代|古时|古老|昔日|当年|那时候|旧时|旧式|早年",
    _re.I,
)
_ERA_MODERN_RE = _re.compile(
    r"\b(today|nowadays|modern|present[- ]?day|these days|21st century|now)\b|"
    r"如今|现在|现今|当今|今天|当代|现代",
    _re.I,
)
_ERA_FUTURE_RE = _re.compile(r"\bin the future\b|未来|将来", _re.I)


def _page_era(outline: BookOutline, page: PageSpec) -> tuple[str, str]:
    """返回本页 (年代kind, 年代label)。kind ∈ {past, modern, contrast, future, ancient, ""}。

    优先信 AI 的 page.era；否则保守地用本页文字推断（只在出现【过去】标记时才判定，
    避免对当代科普里的"today/now/future"等惯用语误触发）。label 用于 prompt 文案。
    """
    raw = (getattr(page, "era", "") or "").strip()
    low = raw.lower()
    if raw:
        if "contrast" in low or "both" in low or "vs" in low or "对比" in low:
            return "contrast", raw
        if any(k in low for k in ("ancient", "古代", "古时")):
            return "ancient", raw
        if any(k in low for k in ("past", "ago", "old", "history", "historical",
                                  "schoolhouse", "过去", "从前", "以前", "旧")):
            return "past", raw
        if "future" in low or "未来" in low or "将来" in low:
            return "future", raw
        if any(k in low for k in ("modern", "present", "today", "now", "现代", "当代", "如今", "现在")):
            return "modern", raw
        return "", raw  # 给了 era 但归不到类（如空泛），不强行注入
    # 无 AI era：仅当本页文字出现【过去】标记时才判定（最保守，防当代页误触发）。
    txt = _page_full_text(outline, page)
    has_past = bool(_ERA_PAST_RE.search(txt))
    has_modern = bool(_ERA_MODERN_RE.search(txt))
    if has_past and has_modern:
        return "contrast", "本页同时对比【过去】与【现在】"
    if has_past:
        return "past", "本页描绘的是【过去/历史】年代"
    return "", ""


def _era_lock_note(outline: BookOutline, page: PageSpec) -> tuple[str, str]:
    """本页若有明确年代设定，返回 (年代锁文案, kind)。

    门槛（保守·防伤其它书）：仅当本书是非虚构科普，或 AI 已给本页 era 标签时才生效。
    """
    if page.page_type == "cover" or page.index == 0:
        return "", ""
    has_ai_era = bool((getattr(page, "era", "") or "").strip())
    if not (_is_nonfiction(outline) or has_ai_era):
        return "", ""
    kind, label = _page_era(outline, page)
    if not kind:
        return "", ""
    head = f"【时代/年代锁·最高优先】本页年代＝{label}。"
    body_common = (
        "建筑/布景/家具/器物/服装/科技必须与该年代一致；其它现代窗、瓷砖、现代课桌椅等冲突描述以本锁为准。"
    )
    if kind in ("past", "ancient"):
        body = (
            "画成过去年代的旧式建筑、陈设、器物和衣着；绝不混入现代教室/瓷砖/电子屏/平板/塑料家具。"
        )
    elif kind == "contrast":
        body = (
            "本页是【今昔对比】：把【过去】与【现在】两种样貌并置同框对比（如左旧右新/一侧旧式一侧现代），"
            "过去区旧式、现代区当代，界限清楚、各自年代正确。"
        )
    elif kind == "future":
        body = "画成未来感样貌；绝不画成过去或当下的旧样。"
    else:  # modern
        body = "画成当代/现代样貌；绝不画成过去的旧式样貌。"
    # 同一年代的相邻页保持连贯（替代被关闭的现代地点蓝本，给跨页一致性兜底）。
    tail = "同一年代页风格连贯。"
    return head + body + body_common + tail, kind
...
def build_cn_page_prompt(
    ...
    *,
    ...
    # 1.95) NF 分页收口（用户拍板 2026-06-11）：仅【封面】带双主角作系列 IP 品牌；
    #   所有正文页(P1-P7)剔除 Mia/Tommy，呈现该主题/年代真实人物与场景。
    if _nf_body_page(page, outline):
        cast = [c for c in cast if c.get("key") not in ("mia", "tommy")]
    ...
    if is_cover:
        ...
        cover_scene_override = (getattr(page, "scene_cn", "") or getattr(page, "scene", "") or "").strip()
        cover_action = (
        ...
        scene_cn = (
            ...
            + (cover_scene_override.rstrip("。") + "。" if cover_scene_override else "")
            ...
            "人物适度偏置于画面一侧，根据场景自然选择标题安全区位置（上方天空/明亮墙面/窗光旁/一侧浅色墙面均可），"
            "标题区必须明亮、自然、有原生场景质感，绝不固定成深色顶部空带；其余画面由主体与环境充实饱满地填满、不空旷。"
        )
    else:
        if official_has_scene:
    ...
    if not is_cover:
        if _nf_body_page(page, outline):
            ...
            scene_cn = (
                ...
                "不出现系列主角 Mia/Tommy（他俩仅作为连续系列 IP 出现在【封面】，正文内页一律不出场）；"
    ...
    if is_cover and official_has_scene:
        ...
        if off:
            scene_cn = (
                ...
                "画风仍" + _STYLE_WORD_CN + "，不照搬官方文字里的画风/负面词）】：" + off
            )
    ...
    # 2.65) 通用文本理解层：本页年代锁（科普/历史今昔对比）。先算出 kind，下面据此决定
    #   是否关闭"现代地点布景蓝本/现代环境提示"（过去/对比页会被它们拽成现代，互相打架）。
    _era_note, _era_kind = _era_lock_note(outline, page)
    _era_historical = _era_kind in ("past", "ancient", "contrast")

    # 2.7) 场景布景连续层：同一地点连续多页 → 布景/建筑/配色完全一致（修 L3#9 走廊不连续）
    #   年代锁为【过去/对比】时跳过：地点布景蓝本是固定的【现代】模板，会与"过去年代"冲突，
    #   改由年代锁负责跨页一致（Book06《Schools of the Past》过去页被现代蓝本拽歪的根因）。
    if not is_cover and not _era_historical:
    ...
    # 2.85) 通用文本理解层：把【关键贯穿物件统一】+【本页年代锁】等高优先锁前置进 scene_cn
    #   （→ 进入 ★0 最高优先段，绝不被 4000 截断切掉）。
    _ko_note = _key_object_note(outline, page)
    _props_note = _recurring_props_note(outline, page)
    _ensemble_note = _class_ensemble_note(outline, page)
    _thought_note = _thought_bubble_note(page)
    _science_note = _science_fiction_lock_note(outline, page)
    _era_pop_note = _era_population_note(outline, page, _era_kind)
    _prefix = ""
    if not is_cover and _era_note:
        _prefix += _era_note.rstrip("。") + "。"
    if _era_pop_note:
        _prefix += _era_pop_note.rstrip("。") + "。"
    if _thought_note:
        _prefix += _thought_note.rstrip("。") + "。"
    if _ensemble_note:
        _prefix += _ensemble_note.rstrip("。") + "。"
    if _ko_note:
        _prefix += _ko_note.rstrip("。") + "。"
    if _props_note:
        _prefix += _props_note.rstrip("。") + "。"
    if _science_note:
        _prefix += _science_note.rstrip("。") + "。"
    if _prefix:
        scene_cn = _prefix + scene_cn
    ...
    blank_cn = _blank_text(page.index) if not is_cover else (
        "封面标题安全区根据画面自适应选择：可在上方明亮天空/墙面，也可在一侧窗光旁或浅色墙面，"
        "必须是明亮自然的场景原生区域（有浅色渐变、墙面/天空/窗光质感），绝不固定为深色顶部空带；"
        "该区域无人物、关键道具和任何文字，作画时不要画书名/标题，也不要画纯白色块或空白方框"
    )
    if (not is_cover) and _era_kind == "contrast":
        blank_cn = (
            "今昔分屏/并置页必须从画面最上边缘顶格铺满两侧场景，旧教室/现代教室都延伸到上边缘；"
            "不要另留顶部横条，不要黑色/深色天空或空白带。文字安全区只用面板内部自然浅色墙面、窗光、地面或边角空处；全框都是已绘制场景"
        )
    ...
    # 年代锁为【过去/对比】时关闭通用环境提示：_ENV_HINTS 的"教室/走廊/家居"都是【现代】模板，
    #   会与"过去年代"冲突（Book06 过去页被塞现代教室的根因）。年代锁 + scene_cn 已承载本页旧式布景。
    if _era_historical:
        env_hint = ""
...
def _apply_book_cast(outline, page, official_raw: str, current_refs: int):
    ...
    for r in roles:
        ...
        if desc:
            tag += f"：{desc}"
        tag += ("（全书每次出现都是同一个角色：脸型/发型/发色/肤色/服装/配色完全一致；友善可爱、绝不凶恶吓人）")
        ...
        if is_animal_role(r):
            ...
            tag += (f"{_sex}成熟成人脸庞与成人身材比例，明显高于 10 岁儿童（约 4:3、孩子头顶到其肩部），"
                    "绝不画成小孩/儿童/青少年。"
                    + (f"必须是成年{_gcn}，绝不画成另一性别。" if _gcn else ""))
        ...
        #   保证不仅锚图对、成图页也不会把 Ben 画成 Tommy/Mia 的翻版。颜色与锚图共用同一确定性指派。
            ...
            tag += (f"【{r.display} 是一个全新独立的国际化儿童角色】与主角 Mia、Tommy 完全不同的另一个孩子，"
                    ...
                    f"与本书同款" + _STYLE_WORD_CN + "画风（不要跑成别的风格）；外貌国际化/可外籍、多元长相，"
    ...
    *, is_cover: bool, title: str, scene_cn: str, cast: list[dict], ip_age: int,
    ...
    if (not is_cover) and scene_cn.strip():
        sec.append(
            ...
            + "以上是画面【主体】、占主要面积、有故事张力地演出，绝不退化成主角呆站/摆拍/只露两张脸。"
        )
    ...
    # 7·材质 / 最新色彩流程（来自 Step 2b 精修流程 SSOT，同步进初生成 prompt，便于 dry-run/渲染自检）。
    if _STYLE_IS_CLEAN:
        sec.append(
            "【7·材质/色彩流程】干净现代绘本插画；clean illustration；smooth shading；refined edges；"
            "smooth gradients；minimal texture；high clarity；无墨线/无纸纹/无水痕；"
            "no noise/grain/artifacts/blotchy details。"
        )
    else:
        sec.append(
            "【7·材质/色彩流程】纸上手绘水彩；clean illustration；smooth shading；"
            "minimal texture；high clarity；no noise/grain/artifacts/blotchy details。"
        )
    # 8·视角/机位
    if is_cover:
    ...
    # 11·留白：使用调用方已按 cover / split-panel / 普通页收口过的场景原生文字区说明。
    if blank_cn:
        sec.append(f"【11·留白】{blank_cn.rstrip('。')}。")
    # 12·层次/景深
    ...
    # 17·色彩（精简：英文画风段已详述；这里只留中文一句锚点·按画风档切换）
    if _STYLE_IS_CLEAN:
        sec.append("【17·色彩】明亮干净——光滑平涂色块+柔和渐变(smooth gradients)、过渡自然、明亮通透不发灰，"
                   "绝不是生硬撞色块/平涂硬色带/调色板拼贴，也无墨线/纸纹/水痕")
    else:
        sec.append("【17·色彩】清透轻水彩——颜色像水彩自然晕开、柔和过渡、明亮通透不发灰，绝不是生硬色块/平涂色带/调色板拼贴")
    # 18·风格（精简：英文画风段已详述；中文锚点按画风档切换）
    if _STYLE_IS_CLEAN:
        sec.append("【18·风格】干净现代儿童绘本插画——光滑着色(smooth shading)、清晰利落的边缘(refined edges)、"
                   "低纹理高清晰(minimal texture/high clarity)、明亮高调柔光，人物圆润可爱、脸颊干净，"
                   "无墨线痕/无纸纹/无水痕，可高分辨率印刷")
    else:
    ...
    elif _STYLE_IS_CLEAN:
        sec.append("【19·参考画风】贴合样图的干净现代绘本插画（光滑、低纹理、高清晰）")
    else:
...
def _build_negative_concise(*, cast: list[dict], page_text: str, ip_age: int,
    ...
    2026-06-11 精简（防 4000 截断·根因修复）：本段在 prompt 尾部，最易被下游截掉，
    故压缩措辞、去掉与正向/front-load 重复的长描述（如整段 MIA_HAIR_LOCK 重述），
    在【不丢任何一类禁忌概念】前提下显著缩短，让防分身/防撞色等负向锁真正存活于截断窗口内。
    """
    neg = [
        "任何文字/字母/数字/书名/水印/logo；",
        "主角在空旷/无关背景里呆站、并排摆拍或被晾成远景路人而脱离本页关键道具/动作/场景；"
        "跳题/省略本页情节；非虚构页缺该主题标志道具/场景；",
        "为凑场景凭空添加、与剧情无关的陌生路人/围观人群/多余配角（未点名者不应出现）；",
        "细碎噪点/高频纹理/脏污颗粒/斑驳色块/密集小装饰/画面脏乱；",
        "生硬色块/平涂色带/调色板色卡/色轮拼贴/突兀撞色块（颜色应像水彩柔和过渡）；",
        "纯白色块/白色方框/人工硬边留白；顶部平涂空色带、人物头顶被齐切、书架/墙/天花在头顶突变单色；",
        ...
        "Mia 丸子头/发髻/half-up/披散/低马尾/侧马尾/双马尾/颅顶超高马尾/非紫色发圈/戴眼镜（须后脑中高位马尾+紫发圈）；"
        "Tommy 长发/马尾/戴眼镜；主角被画成成人或幼儿；",
        "配角超过 2 名、占比过大（>15%）喧宾夺主；",
    ...
    if any((c.get("key") or "").split("_")[0] == "tommy" for c in cast):
        neg.append(
            "Tommy 穿深蓝/藏青/navy/钴蓝/teal 上衣、短袖 polo 翻领、牛仔裤、或上衣接近成人警服藏青——均禁"
            "（Tommy 必须【浅天蓝】长袖圆领卫衣 #5FA8D6~#8EC0ED）；"
        )
...
def _build_positive_v3(
    *, is_cover: bool, title: str, scene_cn: str, cast: list[dict], ip_age: int,
    ...
    if _STYLE_IS_CLEAN:
        parts.append(
            "【画风】干净通透的现代儿童绘本插画：光滑平涂色块+柔和渐变(smooth shading/gradients)、清晰利落的边缘(refined edges)、"
            "细节克制、纹理极简(minimal texture)、高清晰(high clarity)，无墨线痕/无纸纹/无水痕；"
            "配色统一目标：中低饱和、通透不发灰、暖中性白平衡，既不过饱和刺眼、也不发灰发闷；明亮柔光、阴影浅淡干净，边缘清晰可高分辨率印刷；"
            "保留适度景深与轻盈体积(反扁平)，但绝非油画厚涂/3D塑料/强烈硬光影；无噪点/无颗粒/无色斑碎纹理、脸颊干净。"
        )
    else:
        parts.append(
