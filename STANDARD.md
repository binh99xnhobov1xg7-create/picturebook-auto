# 绘本制作标准 v1.9（4 件套全量总表）

> **v1.9 升级（2026-06-03 — 全局底层逻辑前置 + 4 交付物工作台 + 批量生产 + 生图精细印刷）**
>
> ### 一、网页结构重构（三层）
> 1. **全局底层逻辑（只读·最前）**：进页面先一眼看清所有 100% 强制硬标准（命名/模板/视觉/字体/IP+年龄/画风构图/平滑/4 件套尺寸规格）。数据来自 `config.py` 单一数据源 `GLOBAL_STANDARDS` / `DELIVERABLE_SPECS`，网页展示与各 builder 同源，杜绝漂移。
> 2. **输入 + AI 抽取**：3 项必填（Title / Level / 故事），其余 AI 自动。
> 3. **4 交付物工作台（4 角标 Tab）**：📖 绘本 PPT / 📝 练习册 / 📄 阅读报告 / 👩‍🏫 教师指南，各自预览 + 单项修改，底部一键产出 4 件套。
> - 顶部 **单本 / 批量** 模式切换。
>
> ### 二、生图（gpt-image-2 · 方案A）
> - 模型：**gpt-image-2（imarouter）**，不再是即梦。
> - 比例/分辨率：gpt-image-2 出 **1536×1024(3:2)** → 居中裁 **4:3** → 放大到美工口径**精细印刷 2000×1500**（默认 Lanczos，可切 Real-ESRGAN）。
> - 画风：**明亮通透低饱和水彩**（替代"莫兰迪"提法）；底部 15–20% 留白。
> - 平滑控制 + 系列默认 IP（无命名主角时 girl→Mia / boy→Tommy）+ 全本发型/服装/配饰逐页一致，均注入每页提示词。
>
> ### 三、IP 年龄映射（美工口径）
> - **L0/1/2 = 8 岁；L3/4 = 10 岁；L5/6 = 12 岁**（L3 由 8 改 10）。单一数据源 `LEVEL_TO_AGE_DEFAULT`。
> - 常驻 IP：MIA / TOMMY / TEACHER KIM（成人）/ WINNIE（猫）/ Dino（品牌）均已注册参考图 + 描述。
>
> ### 四、Worksheet（2+2+2）
> - 固定 6 页 = **2 Vocabulary + 2 Sentence + 2 Reading**（标题不变）。
> - 第 6 页（阅读 #2）按 Level：**L4-6 = 写作/PBL（标题仍为 Reading）**；**L0-3 = 思维导图**。
> - 题型按 Level 分难度；**删除涂色/纯涂圈/纯绘画类**，低龄改看图选词 / 词图连线 / 拼词 / 看图填句。
> - Sentence 页配图：默认**复用绘本图**（page_02 起），可切「不配图」。
>
> ### 五、Reading Report / Teacher's Guide
> - RR：**空白版 / 示例答案版**双模式（示例版每题下方灰色斜体示例答案）。
> - TG：**固定 8 模块顺序**（Lesson Guide → Pre-Reading → During Reading → Post-Reading → Reading Check → Portfolio → Independent Reading → Lesson Close）；**Answer Key 与 Worksheet 同源**（共用 `_worksheet_questions`，构造上逐题一致）。
>
> ### 六、批量生产（`batch_runner.py`）
> - N 个大纲（`===` 分隔，首行 `Title | Level | Book#`）→ N×4 件交付物。
> - 数据隔离 / 并发可配 / 单本超时 30min / 失败重试 + 隔离 / JSON 日志 / 子文件夹或平铺 + 主 ZIP。
> - 绘本图全自动出图，标记「待人工抽查」。
